...
class TestNavBackfill(unittest.TestCase):
    ...
    def test_backfill_historical_nav_success(self, mock_get):
        ...
        # Run backfill
        success = backfill_historical_nav(self.session, self.scheme.id, self.scheme.amfi_code)
        
        self.assertTrue(success)
        
        # Verify NavHistory records were created
        history = self.session.exec(select(NavHistory).where(NavHistory.scheme_id == self.scheme.id)).all()
        ...
        success2 = backfill_historical_nav(self.session, self.scheme.id, self.scheme.amfi_code)
        self.assertTrue(success2)
        history_after = self.session.exec(select(NavHistory).where(NavHistory.scheme_id == self.scheme.id)).all()
